...
test_that("rgb2hsl", {
  ...
  ans <- f(rgb2hsl(grDevices::col2rgb(cols)))
  ...
  ## Alpha handling:
  m <- rbind(grDevices::col2rgb(cols), alpha=runif(length(cols), 0, 255))
  ans2 <- rgb2hsl(m)
  expect_identical(ans2[1:3, ], rgb2hsl(grDevices::col2rgb(cols)))
